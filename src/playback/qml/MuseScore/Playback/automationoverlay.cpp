...
void AutomationOverlay::init(const QVariantList& automationData)
{
    ...
    for (const QVariant& laneVariant : automationData) {
        ...
        for (const QVariant& point : pointsList) {
            ...
            if (pointMap.value("isSelected").toBool()) {
                m_editedPoint = p;
            }
        }
...
void AutomationOverlay::paint(QPainter* qp)
{
    ...
    for (const AutomationLane& lane : m_lanes) {
        paintLane(painter, lane);
    }
}

void AutomationOverlay::paintLane(muse::draw::Painter* painter, const AutomationLane& lane)
{
    // DBG ----
    ...
    for (const Breakpoint& breakpoint : lane.points) {
        paintBreakpoint(painter, breakpoint);
    }
}

void AutomationOverlay::paintBreakpoint(muse::draw::Painter* painter, const Breakpoint& breakpoint)
{
    const bool selected = breakpoint == m_editedPoint;
...
void AutomationOverlay::mousePressEvent(QMouseEvent* event)
{
    ...
    for (const AutomationLane& lane : m_lanes) {
        for (const Breakpoint& point : lane.points) {
            if (point.center == mappedPos) {
                m_editedPoint = point;
                update();
                return;
            }
