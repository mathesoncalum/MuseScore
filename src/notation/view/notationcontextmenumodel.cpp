...
MenuItemList NotationContextMenuModel::makeElementItems()
{
    ...
    if (hitElement) {
        if (hitElement->isText()) {
            // This directs to the tablature styles page for some reason...
            item->setArgs(mu::actions::ActionData::make_arg2<QString, QString>("text-styles", "title"));
            items << item;
            return items;
        }
