...
bool NotesInChordSelectionFilterModel::enabled() const
{
    ...
    // Section should be enabled whenever a (multi-note) Chord exists within the given selection range...
...
{
    if (!multipleIndicesSelected() && !filtered) {
...
{
    if (noteIdx >= NUM_NOTES_IN_CHORD_SELECTION_FILTER_TYPES - 1) {
    ...
    const SelectionFilterTypesVariant variant = m_types.at(noteIdx);
