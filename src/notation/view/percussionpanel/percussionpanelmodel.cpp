...
void PercussionPanelModel::updateSoundTitle()
{
    if (!interaction()) {
    ...
    const InstrumentTrackId instrumentTrackId {
        staff->part()->id(),
        staff->part()->instrumentId(inputState.segment->tick())
    };

    const audio::AudioInputParams& params = audioSettings()->trackInputParams(instrumentTrackId);
