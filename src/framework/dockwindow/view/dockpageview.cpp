...
bool DockPageView::isDockOpen(const QString& dockName) const
{
    ...
    if (!panel || !panel->dockWidget()) {
        return false;
    }

    const bool isCurrentTab = panel->dockWidget() == panel->currentWidgetInFrame();
    return dock ? dock->isOpen() && isCurrentTab : false;
}
...
void DockPageView::setDockOpen(const QString& dockName, bool open)
{
    ...
    if (destinationPanel) {
        ...
        if (KDDockWidgets::DockWidgetQuick* widget = panel->dockWidget()) {
            destinationPanel->setCurrentWidget(widget);
        }
    } else {
        panel->open();
    }
