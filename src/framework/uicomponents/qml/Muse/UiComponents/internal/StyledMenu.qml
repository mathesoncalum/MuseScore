...
MenuView {
    ...
    onModelChanged: {
        ...
        root.menuMetrics.calculate(root.model)
    }

    desiredHeight: {
        var anchorItemHeight = Boolean(root.anchorItem) ? root.anchorItem.height - padding * 2 : Screen.height
    ...
    desiredWidth: Boolean(root.model) ? root.menuMetrics.itemWidth : 0
    ...
    onIsSubMenuOpenChanged: {
        ...
        if (root.isSubMenuOpen && searchField.inputField.focus) {
    ...
    contentItem: PopupContent {
        ...
        Column {
            ...
            anchors.top: root.isPlacedAbove ? undefined : parent.top
            anchors.bottom: root.isPlacedAbove ? parent.bottom : undefined
