...
void MenuView::updateGeometry()
{
    ...
    const QRectF anchorRect = anchorGeometry();
    ...
    if (viewRect.left() < anchorRect.left()) { // The left of this menu overlaps the left of the anchor (doesn't fit)...
        movePos(m_globalPos.x() + anchorRect.left() - viewRect.left(), m_globalPos.y()); // Move to the right
    }
    ...
    const auto doRepositionResize = [&]() { // This gets quite complicated - lambda avoids nesting...
        ...
        if (isSearchingBottom) {
            const qreal bottomOverlap = viewRect.bottom() - anchorRect.bottom();
            if (bottomOverlap > 0) {
        ...
        const qreal desiredY = parentTopLeft.y() - viewRect.height();
        const qreal topOverlap = anchorRect.top() - desiredY;
        ...
        if (isSearchingAbove || isSearchingRight || anchorRect.top() < desiredY) {
            // Place above...
        ...
        if (!isSearching()) {
            ...
            newPopupPos = PopupPosition::Right; // TODO: This is causing problems...
        }
    };

    // When searching a menu - we need to actively preserve the position / resize...
    const bool overlapsBottom = viewRect.bottom() > anchorRect.bottom();
