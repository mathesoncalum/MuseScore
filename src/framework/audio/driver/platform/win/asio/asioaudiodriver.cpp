...
bool AsioAudioDriver::open(const Spec& spec, Spec* activeSpec)
{
    ...
    //! NOTE A driver whose device has been disconnected can still be loaded and initialized,
    //! and can still report channels and buffer sizes, because the driver dll is registered
    //! independently of the hardware. ASIOStart() is the first call that actually fails, so it
    //! must be made here and its result reported to the caller. Otherwise the driver looks open
    //! while no buffer switch ever happens, and playback silently hangs.
    ok = ASIOStart() == ASE_OK;
    if (!ok) {
