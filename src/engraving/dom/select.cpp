...
ChordRest* Selection::lastChordRestInRange(track_idx_t track) const
{
    ...
    Segment* currSeg = m_endSegment->prev1MM();
    const Segment* endSeg = m_startSegment->prev1MM();
    while (currSeg && currSeg != endSeg) {
        if (!currSeg->enabled() || !currSeg->isChordRestType()) {
...
void Selection::appendChord(Chord* chord)
{
    ...
    for (size_t noteIdx = 0; noteIdx < totalNotesInChord; ++noteIdx) {
        ...
        if (/*hack*/ !isSingleNote && !selectionFilter().canSelectNoteIdx(noteIdx, totalNotesInChord, /*hack*/ true)) {
            continue;
        }
...
void Selection::appendNoteFilteredExtras(Note* note)
{
    ...
    if (PartialTie* ipt = note->incomingPartialTie()) {
        appendFiltered(ipt->frontSegment());
    }
    if (PartialTie* opt = note->outgoingPartialTie()) {
...
void Selection::updateState()
{
    ...
    const bool rangeSegsValid = m_startSegment && m_endSegment;
    const bool rangeStavesValid = m_staffStart != muse::nidx && m_staffStart < totalStaves
                                  && m_staffEnd != muse::nidx && m_staffEnd <= totalStaves
                                  && m_staffStart < m_staffEnd;
    const bool rangeIsValid = isRange() && rangeSegsValid && rangeStavesValid;
