...
// Ugly but handy macro...
#define DEFINE_FILTER_TYPE(Name) \
    bool is##Name##Filtered(const Name##SelectionFilterTypes & type) const \
    { \
        if (type == Name##SelectionFilterTypes::NONE \
            || type == Name##SelectionFilterTypes::ALL) { \
            return m_filtered##Name##Types == static_cast<unsigned int>(type); \
        } \
        return m_filtered##Name##Types & static_cast<unsigned int>(type); \
    } \
    \
    void set##Name##Filtered(const Name##SelectionFilterTypes & type, bool filtered) \
    { \
        if (type == Name##SelectionFilterTypes::NONE) { \
            set##Name##Filtered(Name##SelectionFilterTypes::ALL, !filtered); \
            return; \
        } \
        if (filtered) { \
            m_filtered##Name##Types |= static_cast<unsigned int>(type); \
        } else { \
            m_filtered##Name##Types &= ~static_cast<unsigned int>(type); \
        } \
    } \
    \
    unsigned int m_filtered##Name##Types = static_cast<unsigned int>(Name##SelectionFilterTypes::ALL); \
